...
import { refreshUserRole } from '@/utils/refreshUserRole';
...
function MainComponent() {
  ...
  const handleStatusUpdate = async (applicationId, status) => {
    try {
      ...
      // If the application was approved, refresh the user's role
      if (status === 'approved') {
        try {
          console.log('Application approved, refreshing user role...');
          // Use the utility function to refresh the user's role
          const updatedProfile = await refreshUserRole();
          console.log('User role refreshed successfully:', updatedProfile);
        } catch (refreshError) {
          console.error('Error refreshing user role:', refreshError);
          // Don't throw an error here, as the application was still approved
        }
      }
    } catch (err) {
      console.error(err);
