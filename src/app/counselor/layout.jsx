...
export default function CounselorLayout({ children }) {
  ...
  useEffect(() => {
    if (user) {
      ...
      router.push('/account/signin?redirect=/counselor');
    }
  }, [user, userLoading]);
    ...
    return (
      <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-xl shadow-xl p-8 max-w-md w-full">
          ...
          <Link href="/account/signin?redirect=/counselor" className="block w-full bg-green-600 text-white text-center py-2 px-4 rounded-lg hover:bg-green-700">
            Sign In
          </Link>
        </div>
      </div>
