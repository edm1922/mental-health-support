...
export async function GET() {
  try {
    ...
    console.log('Bigint messages API called');
    
    // First get the current user ID
    const { data: userId, error: userIdError } = await supabase.rpc('get_current_user_id');
    
    if (userIdError || !userId) {
      console.error('Error getting current user ID:', userIdError || 'No user ID returned');
    ...
    console.log('Current user ID (bigint):', userId);
    
    // Get messages for the user
    const { data: userMessages, error: messagesError } = await supabase.rpc('get_messages_for_user', { user_id_param: userId });
    ...
    console.error('Error in bigint messages API:', error);
    return NextResponse.json({
      success: false,
      error: error.message,
      sessions: [],
