    ...
    <Link href={href} className="group block h-full">
      ...
      >
        ...
        >
          {/* Tag label */}
          <div className="mb-4">
            <span className={`inline-flex items-center gap-2 ${tagColorClasses[color]} text-xs px-3 py-1 rounded-full font-medium`}>
              {title}
            </span>
          </div>

          <div className={`transform transition-transform duration-300 group-hover:scale-105 ${isWiggling ? 'animate-wiggle' : ''} mb-4`}>
            {icon}
          </div>

          <h3 className="mb-3 text-xl font-semibold text-gray-900 font-heading">{title}</h3>
          ...
          <span className={`mt-auto inline-flex items-center text-sm font-medium ${colorClasses[color]} bg-white/50 backdrop-blur-sm px-3 py-1.5 rounded-full self-start`}>
            {actionText}
      ...
      <div className="md:col-span-6 lg:col-span-7">
        <div className="grid grid-cols-1 gap-6 h-full md:grid-cols-2">
          <ActionCard
            ...
            description="View and manage your upcoming counseling sessions and availability."
